...
public class SeminarAction extends MeetingAction {
	...
	/**
	 * Saves or updates the seminar to the database when there is no collision
	 * or otherwise displays an error message.
	 * 
	 * @return the result string.
	 */
	public String checkAndSave() {
		collisionList = seminarService.getCollisions(seminar.getId(),
				lecturerId, roomIds, numberOfAppointments, startDate, endDate);
	...
	public String save() {
		try {
			seminarService.saveOrUpdateSeminar(seminar.getId(), meetingName,
					lecturerId, roomIds, numberOfAppointments, startDate,
					endDate);
